...
const HereWhy = () => {
  ...
  const getCardSize = (position, screenSize) => {
    const sizes = {
      desktop: { center: { width: 400, height: 400 }, adjacent: { width: 300, height: 300 }, outer: { width: 250, height: 250 } },
      tablet: { center: { width: 320, height: 320 }, adjacent: { width: 240, height: 240 }, outer: { width: 180, height: 180 } },
      mobile: { center: { width: 280, height: 280 }, adjacent: { width: 200, height: 200 }, outer: { width: 150, height: 150 } },
    };
  ...
  const getGlassOverlayStyles = (position, screenSize) => {
    ...
    const base = "absolute left-3 right-3 backdrop-blur-sm border border-white/20 rounded-2xl";
    if (screenSize === 'desktop') return `${base} bottom-4 ${isCenter ? 'p-4' : 'p-3'} bg-white/20`;
    if (screenSize === 'tablet') return `${base} bottom-3 ${isCenter ? 'p-3' : 'p-2'} bg-white/20`;
              ...
              {visibleIndices.map((cardIndex, i) => {
                ...
                const renderCard = (screen) => (
                  <div
                    key={`${card.id}-${screen}`}
                    className={`${
                      screen === 'desktop' ? 'hidden lg:block' : screen === 'tablet' ? 'hidden md:block lg:hidden' : 'block md:hidden'
                  ...
                  >
                    <img
                      src={`https://picsum.photos/id/${card.id}/${getCardSize(position, screen).width}/${getCardSize(position, screen).height}`}
                      alt={card.title}
                      className="w-full h-full object-cover"
                      style={{ borderRadius: '19px' }}
                      draggable={false}
                    />
                    <div className={getGlassOverlayStyles(position, screen)} style={{ borderRadius: '19px' }}>
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <h2 className={getTextStyles(position, screen).title}>{card.title}</h2>
                          <p className={getTextStyles(position, screen).description}>{card.description}</p>
                        </div>
